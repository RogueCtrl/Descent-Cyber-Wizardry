        ...
        try {
            ...
            if (isInTownCamp) {
                ...
                if (positionData) {
                    ...
                    console.log(`Restored party to floor ${(positionData as any).currentFloor} at (${(positionData as any).playerX}, ${(positionData as any).playerY})`);
                }
        ...
        try {
            ...
            if (this.party.addMember(character)) {
                ...
                if (this.party.id !== (assignedTeam as any).id) {
                    console.log(`Switching active party from ${this.party.id} to ${(assignedTeam as any).id}`);
                ...
                this.ui.addMessage(`${character.name} the ${character.race} ${character.class} has been assigned to Strike Team: ${(assignedTeam as any).name}!`);
                this.eventSystem.emit('party-update');
        ...
        if (this.party && this.party.id) {
            ...
            if (savedDungeons.length > 0) {
                ...
                const mostRecent = savedDungeons.sort((a, b) =>
                    new Date((b as any).lastModified).getTime() - new Date((a as any).lastModified).getTime()
                )[0];
