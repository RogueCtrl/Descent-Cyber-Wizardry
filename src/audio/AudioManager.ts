...
export class AudioManager {
  ...
  generateCyberActiveSegment(_state: any): MusicNote[] {
    ...
    for (let i = 0; i < 8; i++) {
      // 2 measures of 4 quarter notes = 8 beats total?
      ...
      segment.push({ freq: baseFreq, duration: 0.25, wave: 'sawtooth', volume: 0.6 });
      segment.push({
        freq: Math.random() > 0.5 ? baseFreq * 3 : baseFreq,
        duration: 0.25,
        wave: 'sawtooth',
        volume: 0.5,
      });
  ...
  playNote(frequency: number, duration: number, waveType: OscillatorType = 'square', startTime: number = 0): void {
    ...
    noteGain.gain.linearRampToValueAtTime(
      0.2,
      this.audioContext.currentTime + startTime + attackTime
    );
    noteGain.gain.exponentialRampToValueAtTime(
      0.1,
      this.audioContext.currentTime + startTime + duration * decayTime
    );
    noteGain.gain.exponentialRampToValueAtTime(
      0.001,
  ...
  fadeToTrack(trackName: string, _fadeTime: number = 1000): void {
    ...
    if (this.audioContext && this.audioContext.state === 'suspended') {
      if (trackName) {
        console.log(
          `🎵 Audio queued: ${this.tracks[trackName]?.name || trackName} (waiting for user interaction)`
        );
        this.queuedTrack = trackName;
      }
      return;
  ...
  playSoundEffect(effectName: string): void {
    ...
    effectGain.gain.exponentialRampToValueAtTime(
      0.001,
  ...
  refreshTracks(): void {
    ...
    this.tracks = {
      town: {
        name: 'Town Theme',
        tempo: 80,
        pattern: (this as any).createTownTheme(),
      },
      dungeon: {
        name: 'Dungeon Exploration',
        tempo: 60,
