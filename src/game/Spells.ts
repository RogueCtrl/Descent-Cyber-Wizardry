        ...
        if (canCastArcane) {
            ...
            for (let level = 1; level <= arcaneSlots.length; level++) {
                if (arcaneSlots[level - 1] > 0) {
                    availableSpells.arcane.push(...(await this.getSpellsBySchoolAndLevel('arcane', level) as any[]));
                }
            }
        }
        ...
        if (canCastDivine) {
            ...
            for (let level = 1; level <= divineSlots.length; level++) {
                if (divineSlots[level - 1] > 0) {
                    availableSpells.divine.push(...(await this.getSpellsBySchoolAndLevel('divine', level) as any[]));
                }
            }
        }
