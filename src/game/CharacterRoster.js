...
class CharacterRoster {
    ...
    getAvailableCharacters() {
        return this.getAllCharacters().filter(character => 
            character.availability === 'available' && character.isAlive
        );
    }
    ...
    getCharactersByClass(className) {
        ...
        return Array.from(characterIds).map(id => this.allCharacters.get(id)).filter(Boolean);
    }
    ...
    validatePartyComposition(characterIds) {
        ...
        if (characterIds.length > 6) {
            return { valid: false, reason: 'Party cannot exceed 6 members' };
        }
    ...
    getStatistics() {
        ...
        this.getAllCharacters().forEach(character => {
            ...
            switch (character.availability) {
                ...
                case 'in_party': stats.inParty++; break;
                case 'camped': stats.camped++; break;
    ...
    loadFromSave(saveData) {
        ...
        if (saveData.parties) {
            saveData.parties.forEach(partyData => {
                ...
                party.members = party.members.map(memberData => {
