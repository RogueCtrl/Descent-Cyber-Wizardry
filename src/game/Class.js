...
class Class {
    static getClassData(className) {
        const classes = {
            ...
            Bishop: {
                ...
                spells: 'both',
                type: 'elite',
                prerequisites: [{ class: 'Priest', level: 4 }, { class: 'Mage', level: 4 }],
            ...
            Samurai: {
                ...
                requirements: { strength: 15, intelligence: 11, piety: 10, vitality: 14, agility: 10, luck: 9 },
                hitDie: 10,
        ...
        if (classData.type === 'elite') {
            ...
            if (classData.prerequisites && classData.prerequisites.length > 0) {
                const hasPrerequisite = classData.prerequisites.some(prereq => {
                    return character.classHistory && character.classHistory.some(history => 
                        history.class === prereq.class && history.maxLevel >= prereq.level
                    );
                });
                
                if (!hasPrerequisite) {
                    const prereqText = classData.prerequisites.map(p => `${p.class} (level ${p.level}+)`).join(' or ');
    ...
    static getSpellSlots(character, spellSchool) {
        ...
        const primaryStat = spellSchool === 'arcane' ? character.attributes.intelligence : character.attributes.piety;
