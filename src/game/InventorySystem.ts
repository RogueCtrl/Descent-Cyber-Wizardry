    ...
    return `
            <div class="inventory-item ${isCyberMode ? 'cyber-enhanced' : ''}" data-item-id="${typeof item === 'object' ? item.id : item}">
                ...
                <div class="item-actions">
                    ${
                      this.canEquipItem(item)
                        ? `<button class="item-action-btn equip-btn" data-item-id="${typeof item === 'object' ? item.id : item}">
                    ...
                    ${
                      this.canUseItem(item)
                        ? `<button class="item-action-btn use-btn" data-item-id="${typeof item === 'object' ? item.id : item}">
  ...
  sortItems(items) {
    return items.sort((a, b) => {
      switch (this.sortBy) {
        ...
        case 'condition':
          const conditionA =
            typeof a === 'object' && a.durability ? a.durability / a.maxDurability : 1;
          const conditionB =
            typeof b === 'object' && b.durability ? b.durability / b.maxDurability : 1;
