...
export class DungeonTest {
  ...
  testBasicDungeonGeneration() {
    ...
    this.assert(
      this.dungeon.currentFloorData.encounters.length >= 5,
      'Should have at least 5 encounters'
    );
    this.assert(
      this.dungeon.currentFloorData.specialSquares.length >= 2,
      'Should have at least 2 special squares'
    );
  ...
  testMazeStructure() {
    ...
    this.assert(
      reachableTiles >= 50,
      `Should have at least 50 reachable tiles, found ${reachableTiles}`
    );
  ...
  testTrapsAndSpecialSquares() {
    ...
    this.assert(
      specialSquares.length >= 2,
      `Should have at least 2 special squares, found ${specialSquares.length}`
    );
  ...
  testEncounterSystem() {
    ...
    encounters.forEach((encounter, index) => {
      ...
      this.assert(
        encounter.x >= 1 && encounter.x <= 19,
        `Encounter ${index} x should be in bounds`
      );
      this.assert(
        encounter.y >= 1 && encounter.y <= 19,
        `Encounter ${index} y should be in bounds`
      );
      this.assert(
        typeof encounter.level === 'number',
        `Encounter ${index} should have numeric level`
      );
      this.assert(encounter.level > 0, `Encounter ${index} level should be positive`);
      this.assert(
        ['normal', 'boss'].includes(encounter.type),
        `Encounter ${index} type should be valid`
      );
      this.assert(
        typeof encounter.triggered === 'boolean',
        `Encounter ${index} should have triggered flag`
      );
    });
  ...
  testViewingSystem() {
    ...
    this.assert(
      typeof this.dungeon.getViewingInfo === 'function',
      'Should have getViewingInfo method'
    );
    ...
    this.assert(
      typeof viewInfo.position.direction === 'number',
      'Position should have numeric direction'
    );
    this.assert(typeof viewInfo.position.floor === 'number', 'Position should have numeric floor');

    // Test direction values
    this.assert(
      ['North', 'East', 'South', 'West'].includes(viewInfo.facing),
      'Facing should be valid direction'
    );
