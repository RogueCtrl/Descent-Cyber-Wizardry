...
import { SpellMemorization } from './SpellMemorization.ts';
 ...
 */
export class MagicTest {
  testResults: any[];
  spellSystem: Spells;
  memorization: SpellMemorization;
  initialized: boolean;
    ...
    try {
      ...
      for (const school of ['arcane', 'divine']) {
        for (let level = 1; level <= 7; level++) {
          ...
          spellCount[school] += (spells as any).length;
          levelCount[school][level] = (spells as any).length;
        }
      }
  ...
  async testSpellMemorization() {
    ...
    try {
      ...
      const spellSelections = {
        arcane: [{ name: 'Magic Missile' }, { name: 'Light' }, { name: 'Fireball' }],
        divine: [],
      };
      ...
      if (!(memorizeResult as any).success) {
        throw new Error(`Memorization failed: ${(memorizeResult as any).reason}`);
      }
      ...
      this.testResults.push({
        ...
        passed: (memorizeResult as any).success && memorizedCount > 0,
  ...
  testSpellCasting() {
    ...
    try {
      ...
      this.testResults.push({
        test: 'Spell Casting',
        passed: (castResult as any).success && spellWorked,
  ...
  runSpecificTest(testName) {
    ...
    switch (testName) {
      ...
      default:
        console.log(
          'Unknown test name. Available tests: database, slots, memorization, casting, elite, highlevel, effects'
        );
        return;
    }
