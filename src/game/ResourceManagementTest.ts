...
class ResourceManagementTest {
  ...
  async testRestSystem() {
    ...
    try {
      ...
      for (let i = 0; i < 10; i++) {
        // Test multiple times to check for encounters
      ...
      this.assert(restCost > 0, 'Temple rest should have a cost', 'Rest System - Cost Calculation');
  ...
  displayTestResults() {
    ...
    console.log(
      `Passed: ${this.passedTests} (${Math.round((this.passedTests / this.totalTests) * 100)}%)`
    );
    console.log(
      `Failed: ${this.failedTests} (${Math.round((this.failedTests / this.totalTests) * 100)}%)`
    );
...
if (typeof window !== 'undefined' && window.location) {
  ...
  (window as any).runResourceManagementTests = async function () {
    const testSuite = new ResourceManagementTest();
    return await testSuite.runAllTests();
  };

  console.log(
    'Resource Management Test Suite loaded. Run window.runResourceManagementTests() to execute.'
  );
} else {
