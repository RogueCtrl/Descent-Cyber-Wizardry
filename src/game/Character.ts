...
export class Character {
  ...
  recalculateHP() {
    ...
    this.maxHP = Math.max(
      1,
      hitDie + vitBonus + (this.level - 1) * (Math.floor(hitDie / 2) + vitBonus)
    );
  }
  ...
  loadFromSave(saveData) {
    ...
    this.classHistory = saveData.classHistory || [
      { class: this.class, level: 1, maxLevel: 1, timestamp: Date.now() },
    ];
  ...
  validateTeamMembership() {
    ...
    if (!isMemorial && !this.partyId) {
      throw new Error(
        `Active character ${this.name} must have partyId - all agents must be part of a Strike Team`
      );
    }
