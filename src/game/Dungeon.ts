...
import type { DungeonFloor, DungeonTile, RoomData, EncounterData, Direction } from '../types/index.ts';
...
export class Dungeon {
  ...
  initializeFloor(floorNumber: number): void {
    ...
    const floor = {
      ...
      monsters: [],
      treasures: [],
      encounters: this.testMode
        ? this.generateTrainingGroundsEncounters()
        : this.generateEncounters(floorNumber, tiles),
    ...
    console.log(
      `Floor ${floorNumber} generated with ${rooms.length} rooms, ${floor.encounters.length} encounters and ${floor.specialSquares.length} special features`
    );
   ...
   */
  generateWizardryMaze(width: number, height: number, floorNumber: number): { tiles: any[][]; rooms: RoomData[] } {
    const tiles = Array(height)
      .fill(null)
      .map(() => Array(width).fill('wall'));
  ...
  generateRooms(width: number, height: number, roomChance: number): RoomData[] {
    ...
    for (let attempts = 0; attempts < maxRooms * 3; attempts++) {
      ...
      const overlaps = rooms.some(
        (room) => this.roomsOverlap(newRoom, room, 2) // 2-tile buffer
      );
   ...
   */
  carveRooms(tiles: any[][], rooms: RoomData[]): void {
    rooms.forEach((room) => {
   ...
   */
  carveMainCorridors(tiles: any[][], width: number, height: number): void {
   ...
   */
  connectRoomsToCorridors(tiles: any[][], rooms: RoomData[]): void {
    rooms.forEach((room) => {
   ...
   */
  createPathToNearestFloor(tiles: any[][], startX: number, startY: number): void {
   ...
   */
  addMazePassages(tiles: any[][], width: number, height: number, complexity: number): void {
   ...
   */
  carveMazePassage(tiles: any[][], startX: number, startY: number, width: number, height: number): void {
    const directions = [
      [0, 1],
      [1, 0],
      [0, -1],
   ...
   */
  generateDoors(tiles: any[][], width: number, height: number, rooms: RoomData[] = []): void {
        ...
        if (tiles[y][x] === 'wall') {
          ...
          if (doorType === 'vertical') {
            ...
            connectsRoomToCorridor =
              (northIsRoom && southIsCorridor) || (southIsRoom && northIsCorridor);
            ...
            connectsRoomToCorridor =
              (eastIsRoom && westIsCorridor) || (westIsRoom && eastIsCorridor);
          }
    ...
    console.log(
      `Generated ${placedCount} room entrance doors from ${potentialDoors.length} valid positions`
    );
  }
   ...
   */
  isValidDoorPosition(tiles: any[][], x: number, y: number): 'horizontal' | 'vertical' | null {
   ...
   */
  addSecretFeatures(tiles: any[][], width: number, height: number, secretChance: number): void {
   ...
   */
  canPlaceSecret(tiles: any[][], x: number, y: number): boolean {
   ...
   * Add trap squares
   */
  addTraps(tiles: any[][], width: number, height: number, trapChance: number, floorNumber: number): void {
   ...
   */
  canPlaceTrap(tiles: any[][], x: number, y: number): boolean {
   ...
   */
  generateSpecialSquares(floorNumber: number): any[] {
    const specialTypes = [
      'healing_fountain',
      'stamina_fountain',
      'poison_fountain',
  ...
  generateSpecialMessage(type: string): string {
    const messages = {
      healing_fountain: TextManager.getText(
        ...
        'A crystal clear fountain bubbles with restorative waters.'
      ),
      stamina_fountain: TextManager.getText(
        'special_stamina_fountain',
        'This fountain glows with energizing light.'
      ),
      poison_fountain: TextManager.getText(
        'special_poison_fountain',
        'The water here has a sickly green tint.'
      ),
      teleporter: TextManager.getText(
        'special_teleporter',
        'Strange runes circle this magical portal.'
      ),
      message_square: TextManager.getText(
        'special_message_square',
        'Ancient text is carved into the floor here.'
      ),
      treasure_chest: TextManager.getText(
        'special_treasure_chest',
        'A sturdy chest sits here, lock gleaming.'
      ),
    };
   ...
   */
  generateJacks(floorNumber: number, tiles: any[][]): Record<string, { x: number; y: number }> {
    const jacks = {};
    const width = tiles[0].length;
    const height = tiles.length;
    const usedPositions = new Set();
   ...
   */
  validateAndFixMaze(tiles: any[][], width: number, height: number): void {
   ...
   */
  setStartPosition(floor: any): void {
    if (this.testMode) {
      ...
      this.playerDirection = 0; // Start facing north
      console.log(
        `Test mode: Player positioned at (${this.playerX}, ${this.playerY}) facing North`
      );
  ...
  isWalkable(x: number, y: number, floor: number | null = null): boolean {
    ...
    const walkableTiles = [
      ...
      'open_door',
      'open_hidden_door',
      'open_secret_passage', // Only OPEN doors are walkable
      'jack_entry',
      'jack_deep', // Jack egress points (nodes/edges)
      'stairs_up',
      'stairs_down', // Legacy compatibility
      'trap_pit_trap',
      'trap_poison_dart',
      'trap_teleport_trap',
      'trap_alarm_trap',
  ...
  movePlayer(direction: 'forward' | 'backward'): boolean {
    ...
    // Apply wrap-around
    newX =
      ((newX % this.currentFloorData.width) + this.currentFloorData.width) %
      this.currentFloorData.width;
    newY =
      ((newY % this.currentFloorData.height) + this.currentFloorData.height) %
      this.currentFloorData.height;
  ...
  triggerTreasureTile(): void {
    ...
    if (this.eventSystem) {
      this.eventSystem.emit('treasure-tile-entered', {
        x: this.playerX,
        y: this.playerY,
        floor: this.currentFloor,
  ...
  triggerTrap(trapTile: string): void {
    ...
    if (this.eventSystem) {
      this.eventSystem.emit('trap-triggered', {
        type: trapType,
        x: this.playerX,
        y: this.playerY,
        floor: this.currentFloor,
  ...
  checkRandomEncounter(): void {
    ...
    if (fixedEncounter) {
      ...
      if (this.eventSystem) {
        this.eventSystem.emit('encounter-triggered', {
  ...
  checkSpecialSquare(): void {
    ...
    if (special) {
      ...
      if (this.eventSystem) {
        this.eventSystem.emit('special-square-found', {
          special: special,
          x: this.playerX,
          y: this.playerY,
          floor: this.currentFloor,
  ...
  getViewingInfo(): any {
    ...
    for (let distance = 1; distance <= viewDistance; distance++) {
      ...
      for (let offset = -2; offset <= 2; offset++) {
        ...
        switch (this.playerDirection) {
          case 0:
            offX += offset;
            break; // North: offset -1 is left (X-1)
          case 1:
            offY += offset;
            break; // East: offset -1 is left (Y-1)
          case 2:
            offX -= offset;
            break; // South: offset -1 is left (X+1)
          case 3:
            offY -= offset;
            break; // West: offset -1 is left (Y+1)
        }
    ...
    for (let distance = 1; distance <= viewDistance; distance++) {
      ...
      switch (this.playerDirection) {
        case 0: // North
          framingLeftX = checkX - 1;
          framingLeftY = checkY;
          framingRightX = checkX + 1;
          framingRightY = checkY;
          break;
        case 1: // East
          framingLeftX = checkX;
          framingLeftY = checkY - 1;
          framingRightX = checkX;
          framingRightY = checkY + 1;
          break;
        case 2: // South
          framingLeftX = checkX + 1;
          framingLeftY = checkY;
      ...
      for (let offset = 1; offset <= 3; offset++) {
        ...
        switch (this.playerDirection) {
          case 0: // North -> Right is X+
            rightX = checkX + offset;
            rightY = checkY;
            break;
          case 1: // East -> Right is Y+
            rightX = checkX;
            rightY = checkY + offset;
            break;
   ...
   */
  addFramingWalls(walls: any[], distance: number, leftX: number, leftY: number, rightX: number, rightY: number): void {
  ...
  openDoor(startX: number | null = null, startY: number | null = null) {
    ...
    if (openState) {
      ...
      if (this.currentFloorData && this.currentFloorData.tiles) {
        if (
          this.currentFloorData.tiles[targetY] &&
          typeof this.currentFloorData.tiles[targetY][targetX] !== 'undefined'
        ) {
