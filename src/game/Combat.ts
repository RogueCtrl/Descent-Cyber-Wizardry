...
export class Combat {
  ...
  async startCombat(playerParty: any, enemyParties: any, surpriseType: 'party' | 'enemies' | null = null) {
    ...
    this.logMessage(
      `Wave ${this.currentEnemyPartyIndex + 1} of ${this.enemyParties.length}`,
      'wave',
      '🌊'
    );

    // List initial enemies
    const enemies = Array.isArray(this.currentEnemyParty)
      ? this.currentEnemyParty
      : [this.currentEnemyParty];
  ...
  advanceToNextEnemyParty() {
    ...
    if (this.currentEnemyPartyIndex < this.enemyParties.length) {
      ...
      this.logMessage(
        `Wave ${this.currentEnemyPartyIndex + 1} of ${this.enemyParties.length} begins!`,
        'wave',
        '⚔️'
      );
  ...
  getCurrentEnemyPartyInfo() {
    return {
      ...
      enemies: Array.isArray(this.currentEnemyParty)
        ? this.currentEnemyParty
        : [this.currentEnemyParty],
    };
  }
  ...
  endCombat() {
    ...
    const alivePartyMembers = this.combatants.filter(
      (c) => c.hasOwnProperty('class') && c.isAlive
    ).length;

    // Check if any enemies are still alive
    const aliveEnemies = this.combatants.filter(
      (c) => !c.hasOwnProperty('class') && c.isAlive
    ).length;
    ...
    const casualties = this.combatants.filter((c) => c.hasOwnProperty('class') && !c.isAlive);
    ...
    if (this.engine?.eventSystem) {
      if (victory) {
        this.logMessage(`The party stands victorious!`, 'victory', '🏆');
        this.engine.eventSystem.emit('combat-ended', {
          victory: true,
        ...
        if (hasDisconnectedCharacters && !hasAliveCombatants) {
          ...
          this.engine.eventSystem.emit('party-defeated', {
            victory: false,
            casualties: casualties,
            disconnectedCharacters: this.disconnectedCharacters,
            totalDefeat: false,
  ...
  async generateLootFromEnemies(defeatedEnemies: any[], rewards: any) {
    ...
    const avgEnemyLevel =
      defeatedEnemies.reduce((sum, enemy) => sum + (enemy.level || 1), 0) / defeatedEnemies.length;
    ...
    for (const enemy of defeatedEnemies) {
      if (Random.percent(lootChance)) {
        try {
          ...
          if (this.engine?.equipment) {
            const enemyLoot = await this.engine.equipment.generateRandomLoot(enemy.level || 1, 1);
  ...
  async resolveAttack(action: any) {
    ...
    if (!target || !target.isAlive) {
      ...
      return {
        success: false,
        message: `${attacker.name || 'Enemy'} attacks but target is invalid`,
      };
    }

    // Determine weapon type for flavor text
    const weapon = attacker.getCurrentWeapon
      ? attacker.getCurrentWeapon()
      : { name: 'claws', type: 'natural' };
    ...
    if (attackRoll >= targetAC) {
      ...
      if (this.engine?.audioManager) {
        if (attacker.hasOwnProperty('class')) {
          // Player attack
          this.engine.audioManager.playSoundEffect('attack');
        } else {
          // Monster attack
          this.engine.audioManager.playSoundEffect('monsterAttack');
        }
      }
      ...
      if (criticalCheck.instant) {
        this.logMessage(
          `💀 DEVASTATING BLOW! ${target.name} is slain instantly!`,
          'critical',
          '💀'
        );
        ...
        if (this.engine?.eventSystem && target.hasOwnProperty('class')) {
          this.engine.eventSystem.emit('character-updated', { character: target });
        }
      ...
      if (criticalCheck.multiplier > 1) {
        ...
        this.logMessage(
          `🎯 CRITICAL HIT! Damage multiplied by ${criticalCheck.multiplier}!`,
          'critical',
          '🎯'
        );
      }
      ...
      if (this.engine?.audioManager) {
        if (target.hasOwnProperty('class')) {
          // Player hit
          this.engine.audioManager.playSoundEffect('hit');
        } else {
          // Monster hit
          this.engine.audioManager.playSoundEffect('monsterHit');
        }
      }

      this.logMessage(
        `💥 ${attacker.name} hits ${target.name} ${hitDescription} for ${finalDamage} damage!`,
        'combat',
        '💥'
      );
      ...
      if (target.currentHP <= 0) {
        ...
        if (this.engine?.audioManager) {
          this.engine.audioManager.playSoundEffect('characterDeath');
        }
        ...
        this.logMessage(
          `❤️ ${target.name} is ${condition} (${target.currentHP}/${target.maxHP} HP)`,
          'status',
          '❤️'
        );
      }
      ...
      if (this.engine?.audioManager) {
        this.engine.audioManager.playSoundEffect('miss');
      }
  ...
  resolveSpell(action: any) {
    ...
    this.logMessage(
      `${spellEmoji} ${caster.name} begins casting ${spell.name}!`,
      'spell',
      spellEmoji
    );
  ...
  async resolveDisconnect(action: any) {
    ...
    if (
      ...
      this.logMessage(
        `❌ ${character.name} cannot ${actionTerm.toLowerCase()} - they are too ${confusedTerm.toLowerCase()}!`,
        'blocked',
        '❌'
      );
      this.logMessage(
        `${confusedTerm} agents cannot execute escape protocols until they receive treatment.`,
        'system',
        '⚠️'
      );
    ...
    this.logMessage(
      `🏃 ${character.name} attempts to ${actionTerm.toLowerCase()}!`,
      'combat',
      '🏃'
    );
  ...
  async handleSuccessfulDisconnect(character: any) {
    ...
    this.logMessage(
      `✅ ${character.name} successfully ${successTerm.toLowerCase()}s from combat!`,
      'disconnect',
      '✅'
    );
    ...
    const confusedTerm = TextManager.getText('character_status_confused', 'Confused');
    this.logMessage(
      `😵‍💫 ${character.name} is ${confusedTerm.toLowerCase()} from the hasty retreat!`,
      'status',
      '😵‍💫'
    );
    ...
    console.log(
      `DEBUG: Setting ${character.name} status to '${character.status}' and adding confused condition`
    );
    ...
    if (this.engine?.eventSystem) {
      this.engine.eventSystem.emit('character-disconnected', {
        character: character,
  ...
  async handleFailedDisconnect(character: any) {
    ...
    if (aliveEnemies.length > 0) {
      ...
      this.logMessage(
        `😵 ${character.name} is knocked ${unconsciousTerm.toLowerCase()}!`,
        'unconscious',
        '😵'
      );
      ...
      if (this.engine?.eventSystem) {
        this.engine.eventSystem.emit('character-updated', { character: character });
      }
  ...
  logMessage(message, type = 'combat', emoji: string | null = null) {
    ...
    if (this.engine?.ui) {
      ...
      this.engine.ui.addMessage(displayMessage, type);
    }
  }
